...
class Lexer:
    ...
    def get_tokens(self):
        ...
        while pos < len(text):
            ...
            if longest_match == 0:
                ...
                token = local_env.get('token', lexeme)
                tokens.append(token)
                pos += longest_match
        return tokens
    ...
    def rules(self):
        ...
        dfa = DFA(syntax_tree)
        rules.append({'regex': r'''[' ' '\t']#''', 'action': r'''return "skip"''', 'dfa': dfa})
        from models.regex_parser import RegexParser
        from models.syntax_tree import SyntaxTree
        from models.dfa import DFA
        # Regla: ['\n']#
        ...
        syntax_tree = SyntaxTree(postfix)
        dfa = DFA(syntax_tree)
        rules.append({'regex': r'''([0-9])+#''', 'action': r'''return int(lexeme)''', 'dfa': dfa})
        from models.regex_parser import RegexParser
        from models.syntax_tree import SyntaxTree
        from models.dfa import DFA
        # Regla: '+'#
        ...
        syntax_tree = SyntaxTree(postfix)
        dfa = DFA(syntax_tree)
        rules.append({'regex': r'''eof#''', 'action': r'''raise Exception("Fin de buffer")''', 'dfa': dfa})
        return rules
